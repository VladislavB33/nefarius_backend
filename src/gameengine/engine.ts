...
export class Engine extends EventEmitter {
    ...
    async start(): Promise<User> {
        ...
        const { initialCoins } = this.#options;
        await Promise.all(this.#players.map(async (player) => player.addCoins(initialCoins)));
   ...
   */
    ...
    // Give initial invention cards to every player
        ...
        await Promise.all(this.#players.map(
            async (player, index) => player.addCards(
    ...
    async #playRound(): Promise<void> {
        const userTurns: Turn[] = await Promise.all(this.#players.map(async (player) => player.turn()));
   ...
   */
    ...
    async #inventionPhase(userTurns: Turn[]): Promise<void> {
        const inventions = await Promise.all(userTurns.map(async (turn, index) => {
            ...
            return this.#players[index].invent(turn.card);
        }));
   ...
   */
    async #playCoinEffect(positive: boolean, count: number, player: Player): Promise<void> {
        return (positive ? player.addCoins(count) : player.takeCoins(count));
    }
